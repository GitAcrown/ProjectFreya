...
class Downloader(threading.Thread):
    ...
    def duration_check(self):
        log.debug("durée {} pour songid {}".format(self.song.duration,
                                                     self.song.id))
        if self.max_duration and self.song.duration > self.max_duration:
            log.debug("songid {} trop long".format(self.song.id))
            raise MaximumLength("songid {} a une durée {} > {}".format(
...
class Audio:
    ...
    async def _create_ffmpeg_player(self, server, filename, local=False):
        ...
        if voice_client is None:
            log.debug("non connecté alors que {}".format(
                server.id))
    ...
    async def _guarantee_downloaded(self, server, url):
        ...
        if server.id not in self.downloaders:  # We don't have a downloader
            log.debug("sid {} non présent, téléchargement".format(
                server.id))
    ...
    async def _join_voice_channel(self, channel):
        ...
        try:
            ...
            log.exception(e)
            raise ConnectTimeout("Timeout atteint pour le channel")
    ...
    async def _play(self, sid, url):
        ...
        if self._valid_playable_url(url) or "[SEARCH:]" in url:
            try:
                ...
                log.warning("Je ne peux pas jouer l'url ci-dessous parce que trop long."
                            "Utilisez {}audioset maxlength pour changer ça.\n\n"
    ...
    @audioset.command(pass_context=True, name="acceptrole")
    @checks.mod_or_permissions(ban_members=True)
    async def accept_role(self, ctx):
        """Active/Désactive l'acceptation du rôle Audio."""
        if self.settings["ROLEP"] == True:
            self.settings["ROLEP"] = False
            self.save_settings()
            await self.bot.say("Rôle désactivé.")
        else:
            self.settings["ROLEP"] = True
            self.save_settings()
            await self.bot.say("Rôle activé.")

    @audioset.command(pass_context=True, name="audiorole")
    @checks.mod_or_permissions(ban_members=True)
    async def audio_role(self, ctx):
        """Crée le rôle musique permettant ainsi d'utiliser le module Audio"""
        server = ctx.message.server
        nom = "Audio"
        r = discord.utils.get(ctx.message.server.roles, name=nom)
        if nom not in [r.name for r in server.roles]:
            await self.bot.say("Aucun rôle Audio n'est présent sur ce serveur, je vais donc créer le rôle **{}**.".format(nom))
            try:
                perms = discord.Permissions.none()
                # Active les permissions voulues (si nécéssaire)
                await self.bot.create_role(server, name=nom, permissions=perms)
                await self.bot.say("Rôle crée ! \nAssurez-vous que les modérateurs soient au dessus de ce rôle.\n Veuillez régler les permissions du rôle.")
                self.settings["ROLEP"] = True
                self.save_settings()
                try:
    ...
    async def audioset_cachemax(self, size: int):
        """Change le cache maximum en MB"""
        if size < self._cache_min():
            await self.bot.say("Le bot est connecté à trop de serveurs"
                               " ce qui m'empêche de vous donner plus de"
                               " {} MB de cache.".format(
                                   self._cache_min()))
            return

        self.settings["MAX_CACHE"] = size
        await self.bot.say("Cache maximum reglé à {} MB.".format(size))
        self.save_settings()

    @audioset.command(name="maxduree")
    @checks.is_owner()
    async def audioset_maxlength(self, length: int):
        """Durée maximum (secondes) pour les liens envoyés"""
        if length <= 0:
            await self.bot.say("Ah ouais, une valeur négative. Quel génie mon dieu...")
            return
        self.settings["MAX_LENGTH"] = length
        await self.bot.say("Durée maximum de {} secondes.".format(length))
        self.save_settings()

    @audioset.command(name="player")
    @checks.is_owner()
    async def audioset_player(self):
        """Change le player entre Ffmpeg et Avconv"""
    ...
    async def audioset_status(self):
        """Active/désactive le changement de statut pour les musiques"""
        self.settings["TITLE_STATUS"] = not self.settings["TITLE_STATUS"]
        if self.settings["TITLE_STATUS"]:
            await self.bot.say("Si un seul serveur joue une musique, alors ça affichera le nom de la musique.")
            # not updating on disable if we say disable means don't mess with it.
            await self._update_bot_status()
        else:
            await self.bot.say("les titres ne s'afficherons plus")
        self.save_settings()
    ...
    async def audioset_volume(self, ctx, percent: int=None):
        ...
        Note : Peut aller théoriquement jusqu'a 200 mais risque de rencontrer des bugs."""
        server = ctx.message.server
        if percent is None:
            vol = self.get_server_settings(server)['VOLUME']
            msg = "Volume désormais à %d%%" % vol
        elif percent >= 0 and percent <= 200:
            self.set_server_setting(server, "VOLUME", percent)
            msg = "Volume désormais à %d." % percent
            if percent > 100:
                msg += "\nAttention: Un volume supérieur à 100 peut causer des bugs (clipping)"
    ...
    async def audioset_vote(self, ctx, percent: int):
        """Pourcentage nécéssaire pour que la plebe skip la musique. 0 pour désactiver."""
        server = ctx.message.server

        if percent < 0:
            await self.bot.say("Ne peut pas être négatif.")
            return
        elif percent > 100:
            percent = 100
        ...
        if percent == 0:
            enabled = False
            await self.bot.say("Vote désactivé.")
        else:
            enabled = True
            await self.bot.say("Pourcentage de vote reglé à {}%".format(percent))
    ...
    async def audiostat_servers(self):
        """Nombre de serveurs activés en Voix."""

        count = self._player_count()

        await self.bot.say("Je joue de la musique sur {} servers.".format(
            count))
    ...
    async def cache_dump(self):
        ...
        await self.bot.say("Effacement de {:.3f} MB de fichiers audio.".format(dumped))

    @cache.command(name="minimum")
    async def cache_minimum(self):
        """Cache minimum reglé."""
        await self.bot.say("Le cache doit être d'au moins {:.3f} MB".format(
            self._cache_min()))

    @cache.command(name="taille")
    async def cache_size(self):
        """Taille du cache."""
        await self.bot.say("Le cache est à {:.3f} MB.".format(
            self._cache_size()))

    @commands.group(pass_context=True, hidden=True, no_pm=True)
    @checks.is_owner()
    async def disconnect(self, ctx):
        """Gestion de deconnection"""
        if ctx.invoked_subcommand is None:
            server = ctx.message.server
    ...
    async def disconnect_all(self):
        """Deconnecte de tout les serveurs vocaux."""
    ...
    @commands.command(name="joinme", hidden=True, pass_context=True, no_pm=True)
    @checks.is_owner()
    async def joinvoice(self, ctx):
        """Rejoint le serveur de l'auteur."""
    ...
    async def play_local(self, ctx, name):
        ...
        if not self.voice_connected(server):
            try:
                ...
                await self.bot.say("Vous devez rejoindre un channel avant.")
                return
            except UnauthorizedConnect:
                await self.bot.say("Je n'ai pas les permissions")
                return
            except UnauthorizedSpeak:
                await self.bot.say("Je n'ai pas les permissions de diffuser")
                return
            else:
                await self._join_voice_channel(voice_channel)
        ...
        if self.currently_downloading(server):
            await self.bot.say("Je télécharge déjà un fichier !")
            return

        lists = self._list_local_playlists()

        if not any(map(lambda l: os.path.split(l)[1] == name, lists)):
            await self.bot.say("Playlist locale inconnue.")
            return
    ...
    async def list_local(self):
        ...
        if local_playlists:
            ...
            msg = msg.strip(", ")
            msg += "```"
            await self.bot.say("Playlists locales :\n{}".format(msg))
        else:
            await self.bot.say("Aucune playlist trouvée.")

    @commands.command(pass_context=True, no_pm=True)
    async def pause(self, ctx):
        """Pause la musique en cours, `[p]resume` pour continuer."""
        server = ctx.message.server
        if not self.voice_connected(server):
            await self.bot.say("Aucun serveur connecté.")
            return
        ...
        if not hasattr(voice_client, 'audio_player'):
            await self.bot.say("Je joue rien, je met rien en pause.")
        elif voice_client.audio_player.is_playing():
            voice_client.audio_player.pause()
            await self.bot.say("Pause.")
        else:
            await self.bot.say("Rien à jouer, rien à mettre en pause.")
    ...
    async def play(self, ctx, *, url_or_search_terms):
        ...
        r = discord.utils.get(ctx.message.server.roles, name="Audio")
        if self.settings["ROLEP"] == True:
            if "Audio" in [r.name for r in user.roles]:
                ...
                if not self.voice_connected(server):
                    try:
                        ...
                        await self.bot.say("Vous devez d'abord rejoindre un salon")
                        return
                    except UnauthorizedConnect:
                        await self.bot.say("je n'ai pas les permissions")
                        return
                ...
                if "." in url:
                    if not self._valid_playable_url(url):
                        await self.bot.say("Ce n'est pas un URL valide.")
                        return
                else:
                    url = url.replace("/", "&#47")
                    url = "[SEARCH:]" + url
                ...
                await self.bot.say("Vous n'avez pas le rôle Audio")
        else:
            url = url_or_search_terms
            server = ctx.message.server
            author = ctx.message.author
    ...
    async def prev(self, ctx):
        ...
        if self.is_playing(server):
            ...
            await self.bot.say("Je reviens en arrière *retourne le temps*")
        else:
            await self.bot.say("Je joue rien.")
    ...
    async def playlist_create(self, ctx, name):
        """Créé une playlist vide"""
        server = ctx.message.server
        author = ctx.message.author
        if not self._valid_playlist_name(name) or len(name) > 25:
            await self.bot.say("Nom invalide")
            return
        ...
        await self.bot.say("Playlist '{}' sauvegardée.".format(name))
    ...
    async def playlist_add(self, ctx, name, url):
        ...
        author = ctx.message.author
        if not self._valid_playlist_name(name) or len(name) > 25:
            await self.bot.say("Nom de playlist invalide")
            return

        if self._valid_playable_url(url):
            try:
                await self.bot.say("Je suis en train de lister les musiques. Il se pourrait que ça met du temps")
            ...
            self._save_playlist(server, name, playlist)
            await self.bot.say("Playlist '{}' sauvegardée. Morceaux: {}".format(
                name, len(songlist)))
        else:
            await self.bot.say("Cette URL ne semble pas valide.")
    ...
    async def playlist_append(self, ctx, name, url):
        ...
        if name not in self._list_playlists(server):
            await self.bot.say("Aucune playlist avec ce nom.")
            return
        ...
        try:
            ...
            await self.bot.say("Vous n'êtes pas l'auteur de la playlist.")
        except InvalidURL:
            await self.bot.say("Invalidde.")
        else:
            await self.bot.say("Fait.")

    @playlist.command(pass_context=True, no_pm=True, name="extend")
    async def playlist_extend(self, ctx, playlist_url_or_name):
        """Permet d'étendre une playlist avec une autre playlist"""
        # Need better wording ^
        await self.bot.say("Pas encore implémenté.")

    @playlist.command(pass_context=True, no_pm=True, name="list")
    async def playlist_list(self, ctx):
        """Affiche une liste des playlist disponibles"""
        ...
        if files:
            ...
            msg = msg.strip(", ")
            msg += "```"
            await self.bot.say("Playlists disponibles:\n{}".format(msg))
        else:
            await self.bot.say("Aucune playlists.")

    @playlist.command(pass_context=True, no_pm=True, name="queue")
    async def playlist_queue(self, ctx, url):
        """Ajoute une musique à la boucle.

        Ne télécharge PAS localement le fichier."""
        server = ctx.message.server
        if not self.voice_connected(server):
            await self.bot.say("Je ne suis pas connecté.")
            return

        # We are connected somewhere
        if server.id not in self.queue:
            log.debug("Je n'ai pas pu le rajouter")
            raise VoiceNotConnected("Je n'ai pas pu rajouter ce morceau.")
    ...
    async def playlist_remove(self, ctx, name):
        ...
        if self._playlist_exists(server, name):
            ...
            await self.bot.say("Playlist invalide.")
    ...
    async def playlist_start(self, ctx, name):
        ...
        if voice_channel is None:
            await self.bot.say("Je dois d'abord être dans un salon vocal.")
            return
        ...
        if self._playlist_exists(server, name):
            if not self.voice_connected(server):
                try:
                    ...
                    await self.bot.say("Vous devez être dans un salon.")
                    return
                except UnauthorizedConnect:
                    await self.bot.say("Je n'ai pas les autorisations.")
                    return
                except UnauthorizedSpeak:
                    await self.bot.say("Je n'ai pas l'autorisation de parler.")
                    return
                else:
                    await self._join_voice_channel(voice_channel)
            self._clear_queue(server)
            ...
            await self.bot.say("Playlist rajoutée à la boucle.")
        else:
            await self.bot.say("Playlist Invalide.")

    @playlist.command(pass_context=True, no_pm=True, name="mix")
    async def playlist_start_mix(self, ctx, name):
        """Mélange une playlist puis la place dans la boucle."""
    ...
    async def _queue(self, ctx, *, url=None):
        """Ajoute un morceau (depuis un lien) à la boucle en cours. [p] queue help pour en savoir plus

        Si vous utilisez cette commande lorsqu'une musique joue déjà, vous serez rajouté à la boucle.
        En revanche, si rien ne joue, je jouerais votre son puis remettra la boucle à 0."""
        server = ctx.message.server
        user = ctx.message.author
        r = discord.utils.get(ctx.message.server.roles, name="Audio")
        if self.settings["ROLEP"] == True:
            if "Audio" in [r.name for r in user.roles]:
                ...
                if server.id not in self.queue:
                    log.debug("Something went wrong, we're connected but have no"
                              " queue entry.")
                    raise VoiceNotConnected("Je n'ai pas pu la mettre dans la boucle.")
    ...
    async def _queue_list(self, ctx):
        ...
        if server.id not in self.queue:
            ...
            await self.bot.say("Aucun morceau dans la boucle.")
            return
    ...
    async def repeat(self, ctx):
        """Montre si REPEAT est activé"""
        server = ctx.message.server
        if ctx.invoked_subcommand is None:
            if self.is_playing(server):
                if self.queue[server.id]["REPEAT"]:
                    msg = "La boucle se répète."
                else:
                    msg = "La boucle ne se répete pas."
                await self.bot.say(msg)
                await self.bot.say("Faîtes `{}repeat toggle` pour changer ça.".format(ctx.prefix))
            else:
    ...
    async def repeat_toggle(self, ctx):
        """Active/Désactive le Repeat."""
        server = ctx.message.server
        if not self.is_playing(server):
            await self.bot.say("Je n'ai aucun paramètre comme ça")
            return
        ...
        if repeat:
            ...
            await self.bot.say("Désactvié.")

    @commands.command(pass_context=True, no_pm=True)
    async def resume(self, ctx):
        """Relance une musique arrêté"""
        server = ctx.message.server
        if not self.voice_connected(server):
            await self.bot.say("Aucun salon connecté.")
            return
    ...
    async def _shuffle(self, ctx):
        """Mélange la boucle en cours"""
        server = ctx.message.server
        if server.id not in self.queue:
            await self.bot.say("Je n'ai pas de boucle à mélanger.")
            return

        self._shuffle_queue(server)
        self._shuffle_temp_queue(server)

        await self.bot.say("Mélange effectué.")

    @commands.command(pass_context=True, aliases=["next"], no_pm=True)
    async def skip(self, ctx):
        """Saute un morceau ou vous rajoute dans le vote pour sauter le morceau (Si non Modo).
        Modos et Admins ne sont pas comptés dans le vote."""
        msg = ctx.message
        server = ctx.message.server
        user = ctx.message.author
        r = discord.utils.get(ctx.message.server.roles, name="Audio")
        if self.settings["ROLEP"]:
            if "Audio" in [r.name for r in user.roles]:
                if self.is_playing(server):
                    ...
                    if msg.author.voice_channel == vchan:
                        if self.can_instaskip(msg.author):
                            ...
                            if vote >= thresh:
                                ...
                                await self.bot.say("Pourcentage de vote atteint. **Skip...**")
                                return
                            else:
                                reply += " Votes: %d/%d" % (num_votes, num_members)
                                reply += " (%d%% de %d%% nécéssaire)" % (vote, thresh)
                            await self.bot.reply(reply)
                    else:
                        await self.bot.reply("Vous n'êtes pas dans le channel.")
                else:
                    await self.bot.say("Impossible, je ne joue rien.")
            else:
                await self.bot.say("Vous n'avez pas le rôle Audio")
        else:
            if self.is_playing(server):
    ...
    async def infos(self, ctx):
        """Informations a propos de la musique."""
        server = ctx.message.server
        if not self.is_playing(server):
            await self.bot.say("Je n'en joue pas sur ce serveur.")
            return
    ...
    async def stop(self, ctx):
        ...
        if self.is_playing(server):
            if self.can_instaskip(ctx.message.author):
                ...
                await self.bot.say("Vous ne pouvez pas stopper la musique si d'autre personnes sont sur le canal ! utilisez 'vote' plutôt.")
        else:
    ...
    async def yt_search(self, ctx, *, search_terms: str):
        """Cherche et joue une vidéo depuis Youtube"""
        server = ctx.message.server
        user = ctx.message.author
        r = discord.utils.get(ctx.message.server.roles, name="Audio")
        if self.settings["ROLEP"]:
...
def setup(bot):
    ...
    if youtube_dl is None:
        raise RuntimeError("Vous devez installer youtube_dl avec `pip3 install youtube_dl`")
    if opus is False:
        raise RuntimeError(
            "La Librairie Opus doit correspondre avec votre installation Python (32 ou 64 bits)")
    elif opus is None:
        raise RuntimeError(
            "Vous devez installer ffmpeg et opus. Voir \"https://github.com/"
            "Twentysix26/Red-DiscordBot/wiki/Requirements\"")
    try:
        bot.voice_clients
    except AttributeError:
        raise RuntimeError(
            "discord.py est dépassé. Faîtes la MAJ avec\npip3 "
